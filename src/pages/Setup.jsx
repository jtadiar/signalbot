...
export default function Setup({ onComplete }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      // Write config and env via Tauri FS or fall back to localStorage for dev
      try {
        const { writeTextFile, mkdir } = await import('@tauri-apps/plugin-fs');
        await writeTextFile('bot/config.json', JSON.stringify(config, null, 2), { baseDir: 11 });

        const envLines = [
          '# HL Signalbot - generated by setup wizard',
          '',
          `HL_WALLET_ADDRESS=${wallet.trim()}`,
          pkMethod === 'env' ? `HL_PRIVATE_KEY=${privateKey.trim()}` : `HL_PRIVATE_KEY_PATH=~/.config/hl-signalbot/private_key`,
          '',
          `TG_ENABLED=${tgEnabled}`,
          tgEnabled ? `TG_CHAT=${tgChat.trim()}` : '',
          tgEnabled ? `TG_TOKEN_PATH=~/.config/hl-signalbot/tg_token` : '',
          '',
        ].filter(l => l !== undefined).join('\n');
        await writeTextFile('bot/.env', envLines, { baseDir: 11 });

        // Write key file
        if (pkMethod === 'file' && privateKey.trim()) {
          const home = await import('@tauri-apps/api/path').then(m => m.homeDir());
          const keyDir = `${home}.config/hl-signalbot`;
          try { await mkdir(keyDir, { recursive: true }); } catch {}
          await writeTextFile(`${keyDir}/private_key`, privateKey.trim() + '\n');
          if (tgEnabled && tgToken.trim()) {
            await writeTextFile(`${keyDir}/tg_token`, tgToken.trim() + '\n');
          }
        }
      } catch {
        // In dev mode without Tauri, store in localStorage
